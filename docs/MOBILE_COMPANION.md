...
- **Web Push opcional:** conclusão/interrupção de turnos e decisões pendentes. A fila é persistida, utiliza Web Push criptografado e remove inscrições revogadas/expiradas. A notificação contém um texto genérico e link para a conversa, sem prompts ou código.

## Instalar com HTTPS e Tailscale

O acesso HTTP existente continua disponível no navegador. Instalação com suporte offline e push requer contexto seguro. Para usar o endereço privado HTTPS do Tailscale, configure **Tailscale Serve** no computador para encaminhar a origem inteira ao DCC (incluindo `/m`, `/api` e `/auth`). Exemplo para a porta padrão, após conferir a configuração Serve existente:
...
6. Dentro do aplicativo instalado, abra **Configurações → Ativar notificações**. A permissão só é solicitada após esse toque. A disponibilidade depende do navegador/sistema; no iPhone, use o aplicativo instalado.
...
- Push exige `dccd-http` ativo e acesso de saída ao serviço push do navegador. Eventos ficam na fila por até 24 horas, com tentativas espaçadas; não há garantia de entrega instantânea. A validação automatizada não substitui o teste em um iPhone/Android físico.
...
Os testes Rust cobrem o journal de criação, recuperação após reinício, diff com Git real, isolamento de caminhos/symlinks, fila push, identidade VAPID e autenticação/pareamento. Os testes do mobile cobrem reconexão, stream silencioso, revogação e repetição da mesma solicitação.

## Endpoints adicionados

Todos usam a autenticação HTTP existente. Inscrições push exigem identidade de dispositivo pareado.
...
| `GET /api/v1/mobile/push` | Chave pública VAPID e inscrição do dispositivo |
| `POST /api/v1/mobile/push` | Registra `PushSubscription` após consentimento |
| `DELETE /api/v1/mobile/push` | Desativa notificações do dispositivo |
