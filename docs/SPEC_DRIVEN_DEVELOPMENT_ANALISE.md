...
como card estruturado. O veredito agora também registra os checks executados, pulados ou
bloqueados (`RUN` / `SKIPPED` / `BLOCKED`) com evidência, em vez de só resumir critérios.
...
hash atual; o padrão continua `manual`. Além do snapshot atual, cada save também anexa um
evento em `.devcommandcenter/specs/<mission>.validation.history.jsonl`, criando uma trilha
append-only separada do arquivo de estado corrente.
...
No desktop atual, o `remote backend` ainda não está habilitado; portanto o escopo
suportado de setup/reopen é estritamente local neste momento.
...
| Auto-compile setup/reopen | ✅ | Setup/criação e seleção local compilam spec ativa e falha de compile no setup vira warning visível no `WorkspaceSetupReport`; o modo remoto ainda não faz parte do runtime suportado |

### Ledger dos parciais

Este ledger existe para não perdermos o que ainda está deliberadamente incompleto.
