...
const claudeRuntimeMcpCapabilities = {
	...
	mcpSupport: {
		runtimeBridge: {
			providerVersion: "claude-agent-sdk@0.2.126+claude-code@external",
		},
	},
} as const;
...
export const FALLBACK_PROVIDER_CATALOG: ProviderCatalog = {
	providers: [
		...
		},
		{
			...
			label: "Antigravity",
			description:
				"Google's official Antigravity agent through ACP for personal Google accounts.",
			models: PROVIDER_MODEL_REGISTRY.antigravity.map((m) => ({
				id: m.id,
				label: m.label,
				description: m.description,
				recommended: m.recommended,
			...
			health: {
				Degraded: {
					reason:
						"Configure the official Antigravity ACP executable path before starting a session.",
				},
			},
			stable: false,
		},
