...
try {
	...
	for (const [stage, title] of [
		...
		if (["sending", "accepted"].includes(stage)) {
	...
	console.log("Conversation startup smoke passed: prompt continuity, phases, translations, first activity, empty session, responsive layout.");
} finally {
	await browser.close();
}
