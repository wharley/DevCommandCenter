...
const PROVIDER_DESCRIPTION: &str = "Cursor Agent CLI provider with CLI-native session resume and stream-json event parsing.";
...
impl CursorProvider {
	...
	async fn runtime_for_session(&self, session_id: &SessionId) -> Option<Arc<CursorSessionRuntime>> {
		self.runtime
			.sessions
			.lock()
			.await
			...
			if !terminated {
				...
				match exit_result {
					...
					Ok(exit) => {
						...
						let _ = events_tx.send(ProviderEvent::Failed {
							message: message.unwrap_or_else(|| format!("Cursor exited with status {exit}")),
							at,
						});
					}
					Err(error) => {
						...
						let _ = events_tx.send(ProviderEvent::Failed {
							message: message.unwrap_or_else(|| format!("failed to wait for Cursor: {error}")),
							at,
						});
					}
				}
...
fn humanize_cursor_tool_name(tool_name: &str) -> String {
	...
	if lowered.contains("read") {
		...
		"Write".to_string()
	} else if lowered.contains("shell") || lowered.contains("terminal") || lowered.contains("command") {
		"Shell".to_string()
	...
	async fn healthcheck(&self) -> Result<HealthStatus> {
		...
		match parsed.auth.status.as_str() {
			...
			_ => Ok(HealthStatus::Degraded {
				reason: parsed
					.message
					.unwrap_or_else(|| "Cursor Agent authentication could not be verified".to_string()),
			}),
		}
	}
}
...
mod tests {
	...
	fn parses_cursor_model_output_from_json_and_plain_text() {
		let json_models = parse_cursor_models_to_descriptors(r#"{"models":["claude-4-sonnet-thinking","o3"]}"#);
		assert!(json_models.iter().any(|model| model.id == "auto"));
		assert!(json_models.iter().any(|model| model.id == "claude-4-sonnet-thinking"));
		...
		assert!(text_models.iter().any(|model| model.id == "claude-4-sonnet-thinking"));
		assert!(text_models.iter().any(|model| model.id == "o3"));
	}

	#[test]
	fn normalizes_placeholder_model_ids_to_auto() {
		assert_eq!(CursorProvider::normalize_model_arg(Some("cursor-agent")), None);
		assert_eq!(CursorProvider::normalize_model_arg(Some("cursor-editor")), None);
